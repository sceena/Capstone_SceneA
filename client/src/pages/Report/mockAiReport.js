    ...
    question_reports: [
      {
        question_id: 101,
        question: "Redis 캐시를 사용하는 이유를 설명해주세요.",
        answer: "Redis 캐시는 반복 조회 데이터를 메모리에 저장해 DB 접근을 줄이기 위해 사용했습니다. 쇼핑몰 API에서 상품 상세 조회 요청이 몰리는 문제가 있어 TTL 기반 캐싱을 적용했고, 캐시 미스 때만 DB를 조회하도록 바꿨습니다. 적용 후 평균 응답 시간이 약 30% 줄고 DB 커넥션 사용량도 더 안정적으로 유지됐습니다.",
        score: 8.5,
      ...
      },
      {
        question_id: 102,
        question: "트랜잭션 전파 옵션을 설명해주세요.",
        answer: "트랜잭션 전파 옵션은 기존 트랜잭션이 있을 때 새로 호출된 메서드가 그 트랜잭션을 어떻게 사용할지 정하는 설정입니다. REQUIRED는 기존 트랜잭션에 참여하고, REQUIRES_NEW는 기존 트랜잭션과 분리된 새 트랜잭션을 만듭니다. 다만 실제 프로젝트에서 어떤 상황에 적용했는지는 구체적으로 설명이 부족했습니다.",
        score: 4.7,
      ...
      },
      {
        ...
        answer: "주문 목록 조회 API가 데이터 증가 후 느려져 실행 계획을 확인해 보니 특정 조건에서 풀 스캔이 발생하고 있었습니다. 조회 빈도가 높은 주문 상태와 생성일 조건을 기준으로 복합 인덱스를 추가했고, 정렬 조건까지 고려해 인덱스 순서를 조정했습니다. 테스트 데이터 기준 평균 조회 시간이 1초 이상에서 300ms대로 줄었습니다.",
        score: 7.6,
      ...
      },
      {
        ...
        answer: "Spring Security를 적용할 때는 인증과 인가를 분리해서 보는 것이 중요하다고 생각했습니다. 프로젝트에서는 로그인 후 JWT를 발급하고, 요청마다 토큰을 검증해 사용자 정보를 SecurityContext에 저장했습니다. 관리자 API와 일반 사용자 API의 권한을 나눴지만, Refresh Token 재발급이나 토큰 탈취 대응은 더 보완이 필요합니다.",
        score: 7.2,
      ...
      },
      {
        ...
        answer: "장애가 발생하면 먼저 사용자 영향 범위를 확인하고, 로그와 모니터링 지표를 보며 원인을 좁혀갈 것 같습니다. 응답 지연이라면 애플리케이션 로그, DB 커넥션 수, 외부 API 응답 시간을 순서대로 확인하겠습니다. 실제 운영 장애 대응 경험은 많지 않아 개인 프로젝트에 모니터링을 붙이며 보완하고 있습니다.",
        score: 5.9,
