...
const getJobPostingDraftKey = (user) => `${getResumeDraftKey(user)}:job_posting`;
...
export default function ResumeManage() {
  ...
  const [jobPostingText, setJobPostingText] = useState(() => {
    try {
      return localStorage.getItem(getJobPostingDraftKey(user)) || "";
    } catch {
      return "";
    }
  });
  ...
  const handleSave = () => {
    ...
    localStorage.setItem(getJobPostingDraftKey(user), jobPostingText);
    setSaved(true);
  ...
  const handleComplete = () => {
    ...
    if (!jobPostingText.trim()) {
      alert("Fit-Gap 분석에 사용할 채용공고 원문을 입력해 주세요.");
      return;
    }
    ...
    navigate("/mentor/search", {
      state: {
        jobPosting: { rawText: jobPostingText.trim() },
        resumeContent,
      },
    });
  };

  const isValid = items.some(it => it.content.trim().length > 0) && jobPostingText.trim().length > 0;
    ...
    <>
      ...
      <main style={{ maxWidth:820, margin:"0 auto", padding:"36px 5% 80px" }}>
        ...
        <div style={{ background:C.white, borderRadius:16, padding:"24px 28px", border:`1px solid ${C.border}`, marginBottom:20 }}>
          ...
          <div>
            <label style={{ fontSize:13, fontWeight:600, color:C.text, display:"block", marginBottom:7 }}>채용공고 원문</label>
            <textarea
              placeholder={"채용공고의 주요 업무, 자격요건, 우대사항을 그대로 붙여넣어 주세요.\n예: 합류하시면 함께 할 업무입니다... 이런 경험을 가진 분을 찾습니다..."}
              ...
              style={{
                ...
                background:jobPostingFocused ? C.white : C.bg,
                border:`1.5px solid ${jobPostingFocused ? C.navy : "transparent"}`,
                borderRadius:8, fontSize:14, color:C.text,
                lineHeight:1.7, outline:"none", fontFamily:"inherit",
                transition:"all 0.18s", boxSizing:"border-box", resize:"vertical",
              }}
            />
            <p style={{ fontSize:11, color:C.textMuted, marginTop:6, lineHeight:1.5 }}>
              입력한 채용공고 원문은 AI 리포트의 Fit-Gap 분석에서 자소서와 면접 답변을 비교하는 기준으로 사용됩니다.
            </p>
          </div>
        </div>
