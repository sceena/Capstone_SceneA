    ...
    <header style={{ background:C.navy, padding:"0 5%", position:"sticky", top:0, zIndex:100 }}>
      <nav style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:64 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:24 }}>
          {[{l:"대시보드",to:"/dashboard/mentee"},{l:"멘토 탐색",to:"/mentor/search"},{l:"MyPage",to:"/mentee/mypage",bold:true}].map((x,i)=>(
...
const HistoryItem = ({ num, title, wpm, wpmLevel, star, ai, silence, mentor, date, type, isFinal, id, navigate }) => {
  const wc = wpmColor(wpmLevel);
  const reportTag = isFinal ? "최종 리포트" : "1차 리포트";
  const reportColor = isFinal ? C.navy : C.teal;
  return (
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 999, background: `${reportColor}18`, color: reportColor, fontWeight: 600 }}>{reportTag}</span>
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => id && (
              isFinal
                ? navigate(`/report/final/${id}`, { state: { sessionId: id, role: "mentee" } })
                : navigate(`/report/ai/${id}`, { state: { role: "mentee" } })
            )} style={{
...
export default function MenteeMyPage() {
  ...
  const historyAll = apiSessions.map((s, i, arr) => {
    const reportStatus = s.reportStatus ?? s.report_status ?? "";
    const scheduledAt = s.scheduledAt ?? s.scheduled_at ?? s.startedAt ?? s.started_at ?? "";
    return {
      id: s.id,
      num: arr.length - i,
      title: s.title ?? (s.job_category ? `${s.job_category} 모의 면접` : "모의 면접"),
      wpm: s.wpm ?? 0,
      wpmLevel: s.wpmLevel ?? "양호",
      star: s.star ?? "-",
      ai: s.aiScore ?? s.ai_score ?? 0,
      silence: s.silence ?? null,
      mentor: s.mentorName ?? s.mentor_name ?? "",
      date: scheduledAt ? scheduledAt.slice(0, 10).replace(/-/g, ".") : "",
      type: s.sessionType ?? s.session_type ?? "1:1",
      isFinal: reportStatus === "final",
    };
  });
  const finalHistory = historyAll.filter((item) => item.isFinal);
            ...
            <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, overflow: "hidden" }}>
              <div style={{ display: "flex", borderBottom: `1px solid ${C.border}` }}>
                {[
                  ...
                  { k: "unread", l: "최종 리포트", count: finalHistory.length, accent: true },
                ].map(t => (
              ...
              <div style={{ padding: "0 24px 8px" }}>
                {(activeTab === "all" ? historyAll : finalHistory).map((h, i) => (
