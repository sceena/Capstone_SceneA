...
const getReportStatus = (session) => session.reportStatus ?? session.report_status ?? "";
const getAiScore = (session) => session.aiScore ?? session.ai_score ?? "-";
const toDateText = (value) => value ? String(value).slice(5, 10).replace("-", ".") : "";
const toTimeText = (value) => value ? String(value).slice(11, 16) : "";
const SCHEDULED_VISIBILITY_GRACE_MS = 2 * 60 * 60 * 1000;
const isPastScheduledSession = (session) => {
  const status = normalizeStatus(session.status);
  if (status !== "scheduled") return false;
  const scheduledAt = getScheduledAt(session);
  if (!scheduledAt) return false;
  const scheduledTime = new Date(scheduledAt).getTime();
  return Number.isFinite(scheduledTime) && scheduledTime + SCHEDULED_VISIBILITY_GRACE_MS < Date.now();
};
const isVisibleUpcomingSession = (session) => {
  const status = normalizeStatus(session.status);
  if (status === "in_progress") return true;
  if (status !== "scheduled") return false;
  return !isPastScheduledSession(session);
};
...
export default function MenteeDashboard() {
  ...
  useEffect(() => {
    ...
    const unread = sessions.filter(s =>
      ...
      (getReportStatus(s) === "final" || s.tag === "최종 리포트") &&
  ...
  const enterableSessions = sessions.filter(isVisibleUpcomingSession);
  ...
  const history = completedSessions.map(s => {
    const reportStatus = getReportStatus(s);
    const isFinal = reportStatus === "final" || s.tag === "최종 리포트";
    return {
    id: s.id,
    date: toDateText(getScheduledAt(s)),
    title: getSessionTitle(s),
    mentor: getMentorName(s) ? `${getMentorName(s)} 멘토` : "",
    score: getAiScore(s),
    tag: isFinal ? "최종 리포트" : (reportStatus === "first" ? "1차 리포트" : (s.tag ?? "완료")),
    tagColor: isFinal ? C.navy : C.teal,
    isFinal,
    };
  });
      ...
      <main style={{ maxWidth:1100, margin:"0 auto", padding:"36px 5% 60px" }}>
        ...
        {unreadFinals.map(s => (
          <div key={s.id} style={{
            ...
            <button
              onClick={() => navigate(`/report/final/${s.id}`, { state: { sessionId: s.id, role: "mentee" } })}
              style={{
            ...
            {history.length > 0 ? (
              <div>
                {history.map((h, i) => (
                  <HistoryItem key={i} {...h}
                    onView={() => h.isFinal
                      ? navigate(`/report/final/${h.id}`, { state: { sessionId: h.id, role: "mentee" } })
